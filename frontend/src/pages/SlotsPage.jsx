...
function SlotsPage() {
  ...
  const [networkError, setNetworkError] = useState(false)  // ← AJOUT
  ...
  const dateStr = rawSelected
    ? new Date(rawSelected).toLocaleDateString('en-CA')
    : null
    ...
    <div className="min-h-screen bg-[#F5F5F5] relative overflow-hidden">
          ...
          <div className="relative">
            ...
            {showProfileMenu && (
              <div className="absolute right-0 mt-4 w-[200px] sm:w-[250px] bg-white rounded-[20px] shadow-lg p-4 z-50">
                <Link to="/history" className="block w-full px-4 py-3 hover:bg-[#F5F5F5] rounded-xl transition">Historique</Link>
                <Link to="/personal-data" className="block w-full px-4 py-3 hover:bg-[#F5F5F5] rounded-xl transition">Données personnelles</Link>
                <button onClick={() => localStorage.removeItem('token')} className="block w-full text-left px-4 py-3 hover:bg-red-100 text-red-500 rounded-xl transition">
        ...
        {networkError ? (
          <div className="bg-white p-10 rounded-[25px] text-center shadow-lg">
            ...
            <button
              ...
              className="mt-6 px-6 py-3 bg-[#F56B6B] text-white rounded-[15px] font-bold hover:scale-[1.03] transition"
              style={{ fontFamily: 'Playpen Sans' }}
            >
              Réessayer
            </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-8">
            {slotsAffiches.map((slot, index) => (
              <button
                ...
                className={`
                  w-[140px] h-[100px] sm:w-[180px] sm:h-[120px]
                  rounded-[25px] shadow-lg font-bold transition
      ...
      <div className="flex justify-center mt-10 sm:mt-16 relative z-10 pb-8 px-4">
        <Link
          ...
          className={`
            px-8 sm:px-12 py-4 rounded-[18px]
            text-[18px] sm:text-[22px] font-bold shadow-lg transition
